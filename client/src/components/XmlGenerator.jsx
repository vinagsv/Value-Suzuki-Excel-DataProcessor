...
import { processReceiptsClientSide as processReceipts } from '../TallyImport/ReceiptEntries';
...
  RECEIPT:    { label: 'Receipts',   color: '#06b6d4', glow: 'rgba(6,182,212,0.3)' },
...
  
  // New States for Masters Cooldown Logic
...
    RECEIPT:    { fromDate: '', toDate: '', result: null, xmlData: null, mastersXmlData: null, transactionsXmlData: null, error: null },
...
          updateActiveTab('mastersXmlData', null);
          updateActiveTab('transactionsXmlData', null);
          resetCooldowns();
      }
...
        break;
      case 'RECEIPT':
        headers = ['Date', 'Receipt No', 'File Number-Customer Name', 'Amount', 'Mode', 'Type', 'Status', 'Cheque No', 'Dated', 'Remarks'];
        sheetName = "RECEIPTS";
        break;
      case 'CR_JOURNAL':
      case 'DR_JOURNAL':
...
    if (!activeFile && activeTab !== 'RECEIPT') {
      updateActiveTab('error', 'Please upload a file first.');
      return;
    }
    setLoading(true);
    resetCooldowns();
...
              return d >= currentData.fromDate && d <= currentData.toDate;
            });
          }

          if (historyData.length === 0) throw new Error("No receipts found in the database for the selected date range.");

          // Sort historyData to ensure chronological and sequential order
          historyData.sort((a, b) => {
            const dA = new Date(a.date).getTime();
            const dB = new Date(b.date).getTime();
            if (dA !== dB) return dA - dB;
            return parseInt(a.receipt_no || 0) - parseInt(b.receipt_no || 0);
          });
...
          dataSource = historyData.map(item => {
...
        data = await processReceipts(dataSource, formattedFromDate, formattedToDate);
        
        updateActiveTab('result', {
...
          logs: data.logs
        });
        updateActiveTab('mastersXmlData', data.mastersXmlData);
        updateActiveTab('transactionsXmlData', data.transactionsXmlData);
        updateActiveTab('xmlData', data.transactionsXmlData || data.mastersXmlData); 
      } else {
        const formattedFromDate = currentData.fromDate ? currentData.fromDate.replace(/-/g, '') : null;
        const formattedToDate = currentData.toDate ? currentData.toDate.replace(/-/g, '') : null;
...
  const downloadBlob = (xmlString, filename) => {
    if (!xmlString) return;
...
    if (activeTab === 'CR_JOURNAL' || activeTab === 'DR_JOURNAL') {
      downloadBlob(currentTab.xmlData, `Tally_Import_${activeTab}_${currentTab.date.replace(/-/g, '')}.xml`);
    } else {
      downloadBlob(currentTab.xmlData, `${activeTab}_Import.xml`);
    }
  };

  const handleDownloadMasters = () => {
    const currentTab = tabData[activeTab];
    if (currentTab.mastersXmlData) {
      downloadBlob(currentTab.mastersXmlData, `Receipts_Masters_Import.xml`);
    }
...
    const currentTab = tabData[activeTab];
    if (currentTab.transactionsXmlData) {
      downloadBlob(currentTab.transactionsXmlData, `Receipts_Transactions_Import.xml`);
    }
  };
...
  const canProcess = !loading && (activeFile || activeTab === 'RECEIPT');
...
                Upload your data · Extract CSI, BC, VMA, Receipts & Journal Entries
              </p>
            </div>
          </div>
...
                {activeFile ? activeFile.name : `Upload ${TAB_CONFIG[activeTab].label} File`}
              </p>
              <p style={{ margin: '2px 0 0', fontSize: 12.5, color: muted }}>
                {activeFile ? `Loaded for ${currentFileCategory === 'BILLS' ? 'Bills (CSI/BC/VMA)' : TAB_CONFIG[activeTab].label} · Click to replace` : (activeTab === 'RECEIPT' ? 'Drag & drop file, or leave empty to fetch directly from database' : 'Drag & drop or click to browse · .xlsx, .xls, .csv')}
              </p>
            </div>
          </div>
          <div style={{
...
              color: canProcess ? '#fff' : muted,
              fontWeight: 700, fontSize: 15.5,
              cursor: canProcess ? 'pointer' : 'not-allowed',
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10,
              boxShadow: canProcess ? `0 4px 18px ${activeGlow}` : 'none',
              fontFamily: "'DM Sans', sans-serif",
            }}
          >
            {loading ? (
...
                  {activeTab === 'RECEIPT' ? (
                    <>
                      {currentData.mastersXmlData && (
                        <button
                          onClick={handleDownloadMasters}
                          style={{
                            display: 'flex', alignItems: 'center', gap: 7,
...
                        >
                          <Download size={15} /> Download Masters
                        </button>
                      )}
                      
                      <button
                        onClick={handleDownloadTransactions}
                        disabled={!!currentData.mastersXmlData && (!mastersDownloaded || cooldown > 0)}
                        style={{
                          display: 'flex', alignItems: 'center', gap: 7,
                          padding: '8px 18px', borderRadius: 9, border: 'none',
                          background: (!!currentData.mastersXmlData && (!mastersDownloaded || cooldown > 0)) ? muted : activeColor,
                          color: '#fff',
                          fontWeight: 700, fontSize: 13.5, 
                          cursor: (!!currentData.mastersXmlData && (!mastersDownloaded || cooldown > 0)) ? 'not-allowed' : 'pointer',
                          fontFamily: "'DM Sans', sans-serif",
                          boxShadow: (!!currentData.mastersXmlData && (!mastersDownloaded || cooldown > 0)) ? 'none' : `0 2px 10px ${activeGlow}`,
                          transition: 'opacity 0.15s',
                          opacity: (!!currentData.mastersXmlData && (!mastersDownloaded || cooldown > 0)) ? 0.7 : 1
                        }}
                        onMouseOver={e => { if (!(!!currentData.mastersXmlData && (!mastersDownloaded || cooldown > 0))) e.currentTarget.style.opacity = '0.88' }}
                        onMouseOut={e => e.currentTarget.style.opacity = '1'}
                      >
                        <Download size={15} /> 
                        {cooldown > 0 ? `Transactions in ${cooldown}s` : 'Download Transactions'}
                      </button>
                    </>
                  ) : (
                    <button
...
              {/* Stats */}
              <div style={{ display: 'grid', gridTemplateColumns: (activeTab === 'RECEIPT') ? '1fr 1fr 1fr 1fr' : '1fr 1fr 1fr', gap: 0 }}>
                {isJournalTab ? (
                  <>
...
                    { emoji: '👤', label: 'Masters', value: currentData.result.mastersCount || 0, accent: '#3b82f6' },
...
                      padding: '16px 20px',
                      borderRight: i < (arr.length - 1) ? `1px solid ${activeColor}20` : 'none',
                      textAlign: 'center',
                    }}>
...
              {/* Display Skipped and Cancelled Rows specifically for Receipts */}
              {activeTab === 'RECEIPT' && currentData.result.logs && (currentData.result.logs.skipped.length > 0 || currentData.result.logs.cancelled.length > 0) && (
                <div style={{ padding: '16px 20px', borderTop: `1px solid ${activeColor}20`, background: isDark ? '#1e1b24' : '#fffbeb' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, fontWeight: 700, color: '#f59e0b', marginBottom: 10 }}>
                    <AlertCircle size={16} /> Row Processing Details
                  </div>
                  <div style={{ maxHeight: '200px', overflowY: 'auto' }}>
                    <ul style={{ margin: 0, paddingLeft: 20, fontSize: 12.5, color: muted, display: 'flex', flexDirection: 'column', gap: 4, fontFamily: "'DM Mono', monospace" }}>
                      {currentData.result.logs.cancelled.map((msg, idx) => (
                        <li key={`canc-${idx}`} style={{color: '#ef4444'}}>{msg}</li>
                      ))}
                      {currentData.result.logs.skipped.map((msg, idx) => (
                        <li key={`skip-${idx}`}>{msg}</li>
                      ))}
                    </ul>
                  </div>
                </div>
